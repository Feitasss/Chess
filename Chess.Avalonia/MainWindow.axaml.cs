...
public partial class MainWindow : Window
{
    ...
    private ObservableCollection<GameRecord> history;
    ...
    private void DrawBoard(Board board)
    {
        for (int r = 0; r < 8; r++)
        {
            for (int c = 0; c < 8; c++)
            {
                ...
                IImage? source = Images.GetImage(piece);
    ...
    private void DeleteGame_Click(object? sender, RoutedEventArgs e)
    {
        ...
        if (deleteBtn.DataContext is GameRecord gameToDelete)
        {
            ...
            HistoryManager.SaveHistory(history.ToList());
        }
    }
    ...
    {
        var loaded = HistoryManager.LoadHistory();
        loaded.Reverse();

        history = new ObservableCollection<GameRecord>(loaded);
